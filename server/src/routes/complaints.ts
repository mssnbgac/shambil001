...
router.post('/', [
  ...
  try {
    ...
    let defaultRecipients: mongoose.Types.ObjectId[] = [];
    if (category === 'academic') {
      const examOfficers = await User.find({ role: 'exam_officer', isActive: true });
      defaultRecipients = examOfficers.map(officer => officer._id as mongoose.Types.ObjectId);
    } else if (category === 'financial') {
      const accountants = await User.find({ role: 'accountant', isActive: true });
      defaultRecipients = accountants.map(acc => acc._id as mongoose.Types.ObjectId);
    }
    // Always include admin
    const admin = await User.findOne({ role: 'admin', isActive: true });
    if (admin) defaultRecipients.push(admin._id as mongoose.Types.ObjectId);
...
// @desc    Get complaints
// @access  Private
router.get('/', async (req: import('../middleware/auth').AuthRequest, res: import('express').Response) => {
  try {
