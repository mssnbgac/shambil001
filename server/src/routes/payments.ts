...
router.get('/', auth, authorize('admin', 'director', 'accountant'), async (req: import('../middleware/auth').AuthRequest, res: import('express').Response) => {
  try {
...
router.get('/student/:studentId', auth, async (req: import('../middleware/auth').AuthRequest, res: import('express').Response) => {
  try {
...
router.post('/', auth, authorize('admin', 'accountant'), [
  ...
  try {
    ...
    const payment = new Payment({
      ...req.body,
      receiptNumber,
      confirmedBy: req.user ? req.user.id : undefined
    });
