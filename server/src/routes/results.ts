...
router.get('/', auth, authorize('admin', 'director', 'exam_officer', 'teacher'), async (req: import('../middleware/auth').AuthRequest, res: import('express').Response) => {
  try {
...
router.post('/', auth, authorize('admin', 'exam_officer'), [
  ...
  try {
    ...
    if (result) {
      ...
      result.enteredBy = req.user ? req.user.id : undefined;
      result.enteredAt = new Date();
    } else {
      // Create new result
      result = new Result({
        ...req.body,
        enteredBy: req.user ? req.user.id : undefined
      });
    }
