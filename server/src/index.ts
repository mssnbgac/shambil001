...
// Calculate and update class positions
app.post('/api/results/calculate-positions', async (req, res) => {
  try {
    const { academicYear, term, classId } = req.body;
    
    console.log(`🏆 Calculating positions for ${academicYear} - ${term} term, Class ID: ${classId || 'All Classes'}`);
    
    // Build WHERE clause for class filtering
    let whereClause = 'WHERE r.academicYear = ? AND r.term = ? AND r.published = 1';
    let replacements: any[] = [academicYear, term];
    
    if (classId) {
      whereClause += ' AND r.classId = ?';
      replacements.push(classId);
    }
    
    // Get all results for the specified criteria, grouped by class
    const classResults = await sequelize.query(`
      SELECT 
        r.id, r.studentId, r.classId, r.averageScore,
        s.admissionNumber,
      ...
      ${whereClause}
      ORDER BY r.classId, r.averageScore DESC
    `, {
    ...
    const resultsByClass = classResults.reduce((acc: any, result: any) => {
      if (!acc[result.classId]) {
        acc[result.classId] = [];
      }
      acc[result.classId].push(result);
      return acc;
    }, {});
    ...
    for (const [classId, results] of Object.entries(resultsByClass)) {
      const classResultsArray = results as any[];
      const totalStudentsInClass = classResultsArray.length;
      
      console.log(`📊 Processing ${classResultsArray[0].className}: ${totalStudentsInClass} students`);
      
      // Sort by average score (descending) and assign positions
      classResultsArray.sort((a, b) => b.averageScore - a.averageScore);
      
      for (let i = 0; i < classResultsArray.length; i++) {
        const result = classResultsArray[i];
        const position = i + 1;
        
        // Handle ties (same average score gets same position)
        let actualPosition = position;
        if (i > 0 && result.averageScore === classResultsArray[i - 1].averageScore) {
          // Find the position of the first student with this score
          for (let j = i - 1; j >= 0; j--) {
            if (classResultsArray[j].averageScore === result.averageScore) {
              actualPosition = j + 1;
            } else {
              break;
            }
        ...
        await sequelize.query(`
          UPDATE Results 
          SET position = ?, totalStudents = ?, updatedAt = CURRENT_TIMESTAMP
          WHERE id = ?
        `, {
          replacements: [actualPosition, totalStudentsInClass, result.id],
          type: 'UPDATE'
        });
        
        console.log(`✅ ${result.firstName} ${result.lastName} (${result.admissionNumber}): Position ${actualPosition}/${totalStudentsInClass} - Average: ${result.averageScore}%`);
        totalUpdated++;
      }
    }
    
    console.log(`🎯 Successfully updated positions for ${totalUpdated} students`);
    
    res.json({
      message: `Positions calculated successfully for ${Object.keys(resultsByClass).length} classes`,
      positionsUpdated: totalUpdated,
      classesProcessed: Object.keys(resultsByClass).length
    });
...
// Auto-calculate positions when results are created/updated
app.post('/api/results/auto-calculate-positions', async (req, res) => {
  try {
    const { academicYear, term } = req.body;
    
    // Get all classes that have results for this term
    const classesWithResults = await sequelize.query(`
      SELECT DISTINCT r.classId, c.name as className
      FROM Results r
      JOIN Classes c ON r.classId = c.id
      WHERE r.academicYear = ? AND r.term = ? AND r.published = 1
    `, {
    ...
    for (const classInfo of classesWithResults) {
      // Calculate positions for each class
      const response = await fetch(`http://localhost:${process.env.PORT || 4000}/api/results/calculate-positions`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        totalUpdated += result.positionsUpdated || 0;
      }
    }
    
    res.json({
      message: `Auto-calculated positions for ${classesWithResults.length} classes`,
      totalUpdated,
      classesProcessed: classesWithResults.length
    });
    
  } catch (error) {
    console.error('❌ Error in auto-calculate positions:', error);
    res.status(500).json({ message: 'Failed to auto-calculate positions' });
  }
});
app.post('/api/results', async (req, res) => {
  try {
    ...
    console.log('📝 Processing result submission:', {
      student,
      classId,
      academicYear,
      term,
    ...
    if (!classId) {
      // Try to get classId from student record
      console.log('⚠️ No classId provided, fetching from student record...');
      ...
      // Use the classId from student record
      const actualClassId = studentRecord.classId;
      console.log(`✅ Found student's class: ${studentRecord.className} (ID: ${actualClassId})`);
      
      // Continue with the found classId
      return processResults(student, actualClassId, academicYear, term, subjects, remarks, res);
    }
    ...
    if (!subjects || !Array.isArray(subjects) || subjects.length === 0) {
      return res.status(400).json({ message: 'At least one subject result is required' });
    }

    // Process results with provided classId
    return processResults(student, classId, academicYear, term, subjects, remarks, res);
...
async function processResults(student: any, classId: any, academicYear: any, term: any, subjects: any[], remarks: any, res: any) {
  try {
    ...
    // Auto-calculate positions for the class after submitting results
    try {
      console.log('🏆 Auto-calculating class positions...');
      
      // Calculate positions for this specific class
      const positionResponse = await sequelize.query(`
        SELECT 
          r.id, r.studentId, r.averageScore,
          s.admissionNumber,
          u.firstName, u.lastName
        FROM Results r
        JOIN Students s ON r.studentId = s.id
        JOIN Users u ON s.userId = u.id
        WHERE r.classId = ? AND r.academicYear = ? AND r.term = ? AND r.published = 1
        ORDER BY r.averageScore DESC
      `, {
        replacements: [classId, academicYear, term],
        type: 'SELECT'
      }) as any[];
      
      const totalStudentsInClass = positionResponse.length;
      
      // Update positions for all students in this class
      for (let i = 0; i < positionResponse.length; i++) {
        const result = positionResponse[i] as any;
        const position = i + 1;
        
        // Handle ties (same average score gets same position)
        let actualPosition = position;
        if (i > 0 && result.averageScore === positionResponse[i - 1].averageScore) {
          // Find the position of the first student with this score
          for (let j = i - 1; j >= 0; j--) {
            if (positionResponse[j].averageScore === result.averageScore) {
              actualPosition = j + 1;
            } else {
              break;
            }
      ...
      console.log(`🎯 Updated positions for ${totalStudentsInClass} students in class`);
    } catch (positionError) {
      console.error('⚠️ Error calculating positions (non-critical):', positionError);
      // Don't fail the main operation if position calculation fails
    }
...
app.get('/api/comments/templates', async (req, res) => {
  try {
    ...
    const templates = {
      admin: [
        {
          id: 1,
          title: 'Academic Progress Inquiry',
          template: 'Thank you for your inquiry about your child\'s academic progress. We will review their performance and get back to you with a detailed report within 2-3 business days.'
        },
        {
          id: 2,
          title: 'Fee Payment Confirmation',
          template: 'We have received your payment inquiry. Our accounts department will verify the payment status and send you a confirmation receipt within 24 hours.'
        },
        {
          id: 3,
          title: 'General Acknowledgment',
          template: 'Thank you for contacting us. We have received your message and will respond appropriately within 24-48 hours. If this is urgent, please call the school office directly.'
        },
        {
          id: 4,
          title: 'Meeting Request',
          template: 'We would be happy to schedule a meeting to discuss your concerns. Please contact our office at your convenience to arrange a suitable time.'
        }
      ],
      exam_officer: [
        {
          ...
          template: 'Thank you for your inquiry about examination results. We are currently processing results and they will be available soon. You will be notified once they are ready.'
        },
        {
          id: 2,
          title: 'Grade Review Request',
          template: 'We have received your request for grade review. Our examination committee will review the case and respond within 5-7 business days.'
        }
      ]
    };
    
    res.json(templates[role as keyof typeof templates] || []);
  } catch (error) {
...
// Get unread message counts for different roles
app.get('/api/comments/unread-counts', async (req, res) => {
  try {
    const { role } = req.query;
    ...
    if (role === 'admin') {
      whereClause = 'WHERE toRole = ? AND status = ?';
      replacements = ['admin', 'unread'];
    } else if (role === 'exam_officer') {
      whereClause = 'WHERE toRole = ? AND status = ?';
      replacements = ['exam_officer', 'unread'];
    }
    ...
    // Get counts by priority for admin/exam officer
    const priorityCounts = await sequelize.query(`
      SELECT 
        priority,
        COUNT(*) as count
      FROM Comments 
...
app.get('/api/comments', async (req, res) => {
  try {
    ...
    if (role === 'admin') {
      ...
      // Parents can see their own messages - need to get parent name from token
      if (!token) {
        return res.status(401).json({ message: 'Access denied. No token provided.' });
      }
      ...
      try {
        ...
        const [parentUser] = await sequelize.query(`
          SELECT firstName, lastName FROM Users WHERE id = ? AND role = 'parent'
        `, {
          replacements: [decoded.userId],
          type: 'SELECT'
        }) as any[];
    ...
    if (fromRole && fromRole !== 'all' && role !== 'student' && role !== 'parent') {
      if (whereClause) {
...
app.post('/api/comments', async (req, res) => {
  try {
    ...
    // Validation
    if (!subject || !message || !fromRole || !toRole) {
      return res.status(400).json({ 
        message: 'Missing required fields: subject, message, fromRole, and toRole are required' 
      });
    }
    ...
    // Use fromUser/toUser if provided, otherwise fall back to from/to
    const senderName = fromUser || from;
    const recipientName = toUser || to;
    
    // Insert comment into database
    const [commentResult] = await sequelize.query(`
      INSERT INTO Comments (
        fromUser, fromRole, toUser, toRole, subject, message, 
        priority, category, studentAdmission, academicSession, term, status
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'unread')
    `, { 
      replacements: [
        senderName, fromRole, recipientName, toRole, subject, message,
        priority || 'normal', category || 'general', studentAdmission || null, 
        academicSession || null, term || null
      ],
      type: 'INSERT'
    }) as any[];
    
    console.log('✅ Comment created successfully with ID:', commentResult);
...
// Mark parent message as read (when parent views it)
app.patch('/api/comments/:id/mark-read', async (req, res) => {
  try {
    const { id } = req.params;
    const token = req.header('Authorization')?.replace('Bearer ', '');
    ...
    // Update message status to read (only if it belongs to this parent)
    await sequelize.query(`
      UPDATE Comments 
      SET status = 'read', updatedAt = CURRENT_TIMESTAMP
      WHERE id = ? AND fromUser = ? AND fromRole = 'parent'
    `, {
      replacements: [id, parentName],
      type: 'UPDATE'
    });
...
app.post('/api/comments/:id/reply', async (req, res) => {
  try {
    ...
    console.log(`📧 Replying to comment ${id} as ${replierName}`);
    ...
    console.log(`✅ Reply sent successfully for comment ${id}`);
...
// Get message statistics for parents
app.get('/api/comments/parent-stats', async (req, res) => {
  try {
    const token = req.header('Authorization')?.replace('Bearer ', '');
    ...
    const [totalMessages] = await sequelize.query(`
      ...
      WHERE fromUser = ? AND fromRole = 'parent'
    `, {
    ...
    const [repliedMessages] = await sequelize.query(`
      SELECT COUNT(*) as count FROM Comments 
      WHERE fromUser = ? AND fromRole = 'parent' AND reply IS NOT NULL
    `, {
      replacements: [parentName],
      type: 'SELECT'
    }) as any[];
    
    const [pendingMessages] = await sequelize.query(`
      SELECT COUNT(*) as count FROM Comments 
      WHERE fromUser = ? AND fromRole = 'parent' AND reply IS NULL
    `, {
      replacements: [parentName],
      type: 'SELECT'
    }) as any[];
    ...
    const [recentReplies] = await sequelize.query(`
      SELECT COUNT(*) as count FROM Comments 
      WHERE fromUser = ? AND fromRole = 'parent' 
      AND reply IS NOT NULL 
      AND datetime(repliedAt) >= datetime('now', '-24 hours')
    `, {
      replacements: [parentName],
      type: 'SELECT'
    }) as any[];
    ...
    const [avgResponseTime] = await sequelize.query(`
      SELECT AVG(
        (julianday(repliedAt) - julianday(createdAt)) * 24
      ) as avgHours
      FROM Comments 
      WHERE fromUser = ? AND fromRole = 'parent' 
      AND reply IS NOT NULL AND repliedAt IS NOT NULL
    `, {
      replacements: [parentName],
      type: 'SELECT'
    }) as any[];
    ...
    res.json({
      ...
      responseRate: totalMessages.count > 0 ? 
        Math.round((repliedMessages.count / totalMessages.count) * 100) : 0,
      averageResponseTime: avgResponseTime.avgHours ? 
        Math.round(avgResponseTime.avgHours) : null
    });
  } catch (error) {
    console.error('Error fetching parent message stats:', error);
    res.status(500).json({ message: 'Failed to fetch message statistics' });
  }
});

// Search messages for parents
app.get('/api/comments/search', async (req, res) => {
  try {
    ...
    // Build search query
    let whereClause = 'WHERE fromUser = ? AND fromRole = ?';
    let replacements: any[] = [parentName, 'parent'];
    ...
    if (status === 'replied') {
      whereClause += ' AND reply IS NOT NULL';
    } else if (status === 'pending') {
      whereClause += ' AND reply IS NULL';
    }
    
    const searchResults = await sequelize.query(`
      SELECT 
        id, fromUser as "from", fromRole, toUser as "to", toRole, 
        subject, message, priority, category, status, reply, 
        repliedAt, repliedBy, timestamp, createdAt
      FROM Comments
      ${whereClause}
      ORDER BY timestamp DESC
      LIMIT 50
    ...
    res.json({
      ...
      query: { q, category, priority, status }
    });
  } catch (error) {
...
// Fix messages with undefined fromUser (one-time fix)
app.post('/api/admin/fix-parent-messages', async (req, res) => {
  try {
    // Get all messages with 'undefined undefined' as fromUser
    const brokenMessages = await sequelize.query(`
      SELECT id, fromRole FROM Comments 
      WHERE fromUser = 'undefined undefined' AND fromRole = 'parent'
    `, { type: 'SELECT' }) as any[];

    console.log('Found broken messages:', brokenMessages.length);

    // For each broken message, try to find the correct parent name
    for (const message of brokenMessages) {
      // Since we can't determine which parent sent it, we'll need to update manually
      // For now, let's just mark them as needing manual review
      await sequelize.query(`
        UPDATE Comments 
        SET fromUser = 'Parent (Name Missing)' 
        WHERE id = ?
      `, { replacements: [message.id], type: 'UPDATE' });
    }

    res.json({ 
      message: `Fixed ${brokenMessages.length} broken parent messages`,
      fixed: brokenMessages.length 
    });
  } catch (error) {
    console.error('Error fixing parent messages:', error);
    res.status(500).json({ message: 'Failed to fix messages' });
  }
});